    ...
    if (len(body) > 2500) or ("SA Number" in body):
        ...
        try:
            ...
            for index, item in enumerate(parse2):
                if ('user id' in item.lower()):
                    found.append(index)
    
            special_index = found[0]
            print(special_index)         

            try:
                name = ('name', parse2.split([special_index + 1]))
...
def iter_mail(sender_func, mailbox, index, days):
    
    today = datetime.datetime.now()
    last_day = today - datetime.timedelta(days)
    last_day
...
def get_emails(days):
    ...
    print('{} admin emails'.format(len(admin)))
    ...
    '''try:
        scrape, j2 = iter_mail(aaron_to_dict, aaron, j, days)
    ...
    try:
        admin_scrape, j3 = iter_mail(admin_to_dict, admin, j, days)
    ...
    '''try:
        ngrid_scrape, j4 = iter_mail(ngrid_to_dict, ngrid, j3, days)
...
def main():
    output_dict, filename = get_emails(7)
